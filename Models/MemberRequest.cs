    ...
    public class MemberRequest
    {
       ...
       [BsonId]
        ...
        public bool InsertIntoDocument(string databaseName = "mentorsLabDB") {
            IMongoCollection<MemberRequest> collection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<MemberRequest>(GetType().Name); //  ~ Consists of Point 1
        ...
        public void UpdateDocument(MemberRequest newOne, string databaseName = "mentorsLabDB") {
            IMongoCollection<MemberRequest> collection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<MemberRequest>(GetType().Name);
            List<MemberRequest> uList = collection.AsQueryable().Where(p => p.Id == this.Id).ToList();
            BsonDocument filter = new BsonDocument();

            foreach (MemberRequest item in uList) {
        ...
        public MemberRequest SearchFirstOrSingle(Dictionary<string, object> queryDictionary, string databaseName = "mentorsLabDB") {
        ...
        public List<MemberRequest> SearchDocument(Dictionary<string, object> queryDictionary, string databaseName = "mentorsLabDB") {
            // the below statement just obtains the relevent document collection associated with the search
            var selectedCollection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<MemberRequest>(GetType().Name);
        ...
        public bool DeleteFromDocument(Dictionary<string, object> queryDictionary, string databaseName = "mentorsLabDB") {
            // the below statement just obtains the relevent document collection associated with the search
            var selectedCollection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<MemberRequest>(GetType().Name);
            BsonDocument filter = new BsonDocument(); // we create a blank filter
            foreach (string item in queryDictionary.Keys) // we now iterate through our dictionary looking for all key and value types
            ...
            return selectedCollection.DeleteOne(filter).DeletedCount > 0 ? true : false ; // this will use the filter to return a list of only documents that fit that specific filter
        }
