...
namespace MentorAPI.Models {
    public class UserStats
    {
        ...
        [BsonElement]
        public List<string> StartUpsAssignedTo { get; set; }
        #endregion
        ...
        {
            IMongoCollection<UserStats> collection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<UserStats>(GetType().Name); //  ~ Consists of Point 1
        ...
        public void UpdateDocument(UserStats newOne,string databaseName = "mentorsLabDB")
        {
            IMongoCollection<UserStats> collection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<UserStats>(GetType().Name);
            List<UserStats> uList = collection.AsQueryable().Where(p => p.Id == this.Id).ToList();
            BsonDocument filter = new BsonDocument();
            
            foreach (UserStats item in uList)
            {
        ...
        public UserStats SearchFirstOrSingle(Dictionary<string, object> queryDictionary, string databaseName = "mentorsLabDB")
        {
        ...
        public List<UserStats> SearchDocument(Dictionary<string, object> queryDictionary, string databaseName = "mentorsLabDB")
        {
            // the below statement just obtains the relevent document collection associated with the search
            var selectedCollection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<UserStats>(GetType().Name);
