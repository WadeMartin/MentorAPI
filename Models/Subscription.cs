...
namespace MentorAPI.Models {
    public class Subscription
    {
        ...
        [BsonElement]
        public bool StatisticsViewable { get; set; }

        #region CRUD Methods

        public bool InsertIntoDocument(string databaseName = "mentorsLabDB")
        {
            IMongoCollection<Subscription> collection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<Subscription>(GetType().Name); //  ~ Consists of Point 1
        ...
        public void UpdateDocument(Subscription newOne, string databaseName = "mentorsLabDB") {
            IMongoCollection<Subscription> collection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<Subscription>(GetType().Name);
            List<Subscription> uList = collection.AsQueryable().Where(p => p.Id == this.Id).ToList();
            BsonDocument filter = new BsonDocument();

            foreach (Subscription item in uList) {
        ...
        public Subscription SearchFirstOrSingle(Dictionary<string, object> queryDictionary, string databaseName = "mentorsLabDB")
        {
        ...
        public List<Subscription> SearchDocument(Dictionary<string, object> queryDictionary, string databaseName = "mentorsLabDB")
        {
            // the below statement just obtains the relevent document collection associated with the search
            var selectedCollection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<Subscription>(GetType().Name);
        ...
        public void DeleteDocument(string databaseName = "mentorsLabDB") {

            var selectedCollection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<Subscription>(GetType().Name);
            BsonDocument filter = new BsonDocument(); // we create a blank filter
          

                    filter.Add("_id", new ObjectId(this.Id.ToString()));
