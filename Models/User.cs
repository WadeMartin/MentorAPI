...
namespace MentorAPI.Models {
    public class User
    {
        ...
        [BsonElement]
        public string CVDocLoc { get; set; }
        [BsonElement]
        ...
        {
            IMongoCollection<User> collection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<User> (GetType().Name); //  ~ Consists of Point 1
            if (collection.AsQueryable().Where(p => p.Username == this.Username).LongCount() > 0) //  used to check if the current object already exists
            {
        ...
        public bool UpdateDocument(User newOne, string databaseName = "mentorsLabDB") {
            try {
                IMongoCollection<User> collection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<User>(GetType().Name);
                List<User> uList = collection.AsQueryable().Where(p => p.Username == this.Username).ToList();
        ...
        public User SearchFirstOrSingle(Dictionary<string, object> queryDictionary, string databaseName = "mentorsLabDB")
        {
        ...
        public  List<User> SearchDocument(Dictionary<string, object> queryDictionary, string databaseName = "mentorsLabDB")
        {
            // the below statement just obtains the relevent document collection associated with the search
            var selectedCollection = new DatabaseConnection().DatabaseConnect(databaseName).GetCollection<User>(GetType().Name);
